...
class BaseWebObject(unittest.TestCase):
    ...
    def _is_element_enabled(self, element):
        try:
	...
	try:
	    if self._is_element_displayed(element):
    ...
    def _select_dropdown_by_index(self, index, element):
        select = Support.Select(element)
    ...
    def _wait_till_element_is_clickable(self, element, timeout=None):
	self._wait_until(lambda x: self._is_element_clickable(element))
#			 timeout)
